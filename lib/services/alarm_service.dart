...
class AlarmService {
  static void sendAlarm() async {
    ...
    if (user != null) {
      ...
      if (contacts != []) {
        ...
        print("Cannot send alarm without contacts");
      }
    } else {
      print("Cannot send alarm, without a user profile");
    }
  }
  ...
  static String _getAlarmMessage(User user) {
    ...
    message = message +
        "Geburtstag: ${user.birthday.day.toString().padLeft(2, "0")}.${user.birthday.month.toString().padLeft(2, "0")}.${user.birthday.year.toString()}\n";
